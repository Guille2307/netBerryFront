...
export class MytaskeditComponent implements OnInit {
  ...
  get currentTask(): Task {
    const task = this.editMyTaskForm.value as Task;
    return task;
  }
  ...
  refillFom() {
    this.dashboardService.getTaskById(this.id).subscribe((resp: any) => {
      this.editMyTaskForm.reset(resp.task);
    });
  }
  ...
  postAndConfirmation() {
    ...
    if (this.editMyTaskForm.invalid) {
      return;
    }
  ...
  validField(field: string) {
    if (this.editMyTaskForm.get(field)?.invalid && this.FormSubmitted) {
      return true;
    } else {
      return false;
    }
